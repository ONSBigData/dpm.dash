...
server_sens <- function(input, output, session) {
  ...
  shiny::observeEvent(input$create_system_models, {
    ...
    new_sysmods_temp <- lapply(models, function(model) {
      ...
      ## Load/set rates dispersion (single value/data frame)
      disp_val_or_df <- NULL
      ...
      ## Set optional system model parameters
      ...
      if (!is.function(create_system_model)) {
    ...
    sysmod_list(new_sysmods_temp) # Update the reactiveVal for the current set
      ...
      output[[paste0(model_name, "_summary_display")]] <- renderPrint({
        ...
        if (!is.null(sysmod_component) && !is.null(sysmod_component$mean)) {
          ...
          if (!exists("generate_summary") || !is.function(generate_summary)) {
            return("generate_summary() not found")
          }
  ...
  # Observer for deleting a system model setup
  ...
  # Observer for updating delete_sysmod_list choices
  observe({
  ...
  auxData <- shiny::reactive({
      ...
      if (nzchar(ratio_file_input)) {
        ...
        if (file.exists(full_path)) {
          return(tryCatch(utils::read.csv(full_path), error = function(e) {
            warning(paste("Ratio file read error:", e$message))
            ratio_value_input
          }))
        } else {
          warning(paste("Ratio file not found:", full_path, "Using single value."))
          ratio_value_input
        }
      } else {
        ratio_value_input
    ...
    if (input$data_model == "Normal Data Model") {
      ...
      extraData$scale_ratio <- input$scale_ratio
      extraData$count_scaler <- input$count_scaler
    } else if (input$data_model == "Log-Normal Data Model") {
      req(input$counts_uncertainty_file) #
      uncertainty_path <- file.path(data_dir_path, input$counts_uncertainty_file)
      if (!file.exists(uncertainty_path)) {
        shinyWidgets::sendSweetAlert(session = session, title = "File Error", text = paste("Counts uncertainty file not found:", uncertainty_path), type = "error")
  ...
  shiny::observeEvent(input$create_data_model, {
    ...
    updateSelectInput(session, "series_name", selected = "population")
  ...
  shiny::observeEvent(input$plotdm_button, {
    ...
    output$dmModelPlots <- shiny::renderUI({
      plotly::plotlyOutput("plot_dm_model_actual")
    })
  ...
  shiny::observeEvent(input$inspectdm_button1, {
    ...
    output$dmModelInspect1 <- DT::renderDT({
  ...
  output$loadedDataModels <- DT::renderDT({
    ...
    if (length(dataModels) > 0) {
      modelSummaries <- lapply(dataModels, function(dm) {
        ...
        c(
          dm_name = dm$nm_data, series_name = dm$nm_series, dm_type = class(dm)[[1]],
  ...
  output$data_model_checklist <- shiny::renderUI({ # For single fit tab
    ...
    checkboxGroupInput("selected_data_models_fit", "Select Data Models:", choices = data_model_names)
  })
  ...
  shiny::observeEvent(input$fit_account_model, {
    ...
    shiny::withProgress(message = "Fitting Account Model...", value = 0, {
      ...
      mig_res_full <- result %>%
        ...
        dplyr::mutate(age = .data$time - .data$cohort) %>% # Keep original 'age' if it exists, else use calculated
  ...
  output$immigration_estimates_plot <- renderPlotly({
    ...
    p <- ggplot2::ggplot(mig_data_to_plot, ggplot2::aes(
      x = .data$age, ymin = .data$ins.lower,
      y = .data$ins.fitted, ymax = .data$ins.upper
    )) +
      ggplot2::geom_pointrange(fatten = 0.2, col = "darkorange") +
      ggplot2::ylab("Count") +
      ggplot2::ggtitle("Immigration Estimates (Fitted)") +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::theme_minimal()
  ...
  output$emigration_estimates_plot <- renderPlotly({
    ...
    p <- ggplot2::ggplot(mig_data_to_plot, ggplot2::aes(
      x = .data$age, ymin = .data$outs.lower,
      y = .data$outs.fitted, ymax = .data$outs.upper
    )) +
      ggplot2::geom_pointrange(fatten = 0.2, col = "darkorange") +
      ggplot2::ylab("Count") +
      ggplot2::ggtitle("Emigration Estimates (Fitted)") +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::theme_minimal()
  ...
  }) # fitModels is the first subtab of Compare Setups
  ##############################################################################
  ...
  shiny::observeEvent(input$compare_account_model, {
    ...
    shiny::withProgress(message = "Comparing Account Models...", value = 0, {
      ...
      result_1 <- tryCatch(accountTMB::estimate_account(datamods = filtered_data_models_1_comp(), sysmods = filtered_sys_models_1_comp(), seed_in = global_config()$seed_value),
        error = function(e) {
          shinyWidgets::sendSweetAlert(session, title = "Error Model 1", text = e$message, type = "error")
          NULL
        }
      )
      if (is.null(result_1)) {
      ...
      result_2 <- tryCatch(accountTMB::estimate_account(datamods = filtered_data_models_2_comp(), sysmods = filtered_sys_models_2_comp(), seed_in = global_config()$seed_value + 1), # Slightly different seed
        error = function(e) {
          shinyWidgets::sendSweetAlert(session, title = "Error Model 2", text = e$message, type = "error")
          NULL
        }
      )
      if (is.null(result_2)) {
  ...
  observe({
    ...
    if (!is.null(res$pop_combined) && "time" %in% names(res$pop_combined)) {
      ...
      choices_mig <- choices[c(-1)]
      updateSelectInput(session, "time_select_comp", choices = choices, selected = choices[1])
      updateSelectInput(session, "time_select_comp_mig", choices = choices_mig, selected = choices_mig[1])
    }
  ...
  output$comparing_estimates_plot <- plotly::renderPlotly({
    res <- comparison_results()
    req(res, res$pop_combined, input$time_select_comp, input$setup_select)
    ...
    if (input$setup_select == "Setup 1" || input$setup_select == "Both") {
      if (input$compare_select_pop_1 != "None" && input$compare_select_pop_1 %in% colnames(res$full_pop_1)) {
        ...
        p <- p + ggplot2::geom_point(
          data = comp_data_1, ggplot2::aes(x = .data$age, y = compare_val), color = "red", shape = 4, inherit.aes = FALSE
        )
      }
    }
    if (input$setup_select == "Setup 2" || input$setup_select == "Both") {
      if (input$compare_select_pop_2 != "None" && input$compare_select_pop_2 %in% colnames(res$full_pop_2)) {
        ...
        p <- p + ggplot2::geom_point(
          data = comp_data_2, ggplot2::aes(x = .data$age, y = compare_val), color = "blue", shape = 3, inherit.aes = FALSE
        )
      }
    }
    plotly::ggplotly(p)
  ...
  output$comparing_estimates_residuals_plot <- plotly::renderPlotly({
    res <- comparison_results()
    req(res, res$pop_wide, input$time_select_comp, input$residual_type)
  ...
  output$comparing_ins_plot <- plotly::renderPlotly({
    ...
    p <- ggplot2::ggplot(plot_data, ggplot2::aes(
      ...
      ggplot2::geom_ribbon(alpha = 0.3, linetype = "dashed") +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::labs(title = "Immigration Comparison", y = "Count") +
      ggplot2::theme_minimal()
    ...
    if (input$setup_select == "Setup 1" || input$setup_select == "Both") {
      if (input$compare_select_ins_1 != "None" && input$compare_select_ins_1 %in% colnames(res$full_mig_1)) {
        ...
        p <- p + ggplot2::geom_point(data = comp_data_1, aes(x = .data$age, y = compare_val), color = "red", shape = 4, inherit.aes = FALSE)
      }
    }
    if (input$setup_select == "Setup 2" || input$setup_select == "Both") {
      if (input$compare_select_ins_2 != "None" && input$compare_select_ins_2 %in% colnames(res$full_mig_2)) {
        ...
        p <- p + ggplot2::geom_point(data = comp_data_2, aes(x = .data$age, y = compare_val), color = "blue", shape = 3, inherit.aes = FALSE)
      }
    }
    plotly::ggplotly(p)
  })
  ...
  output$comparing_outs_plot <- plotly::renderPlotly({
    ...
    p <- ggplot2::ggplot(plot_data, ggplot2::aes(
      ...
      ggplot2::geom_ribbon(alpha = 0.3, linetype = "dashed") +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::labs(title = "Emigration Comparison", y = "Count") +
      ggplot2::theme_minimal()
  ...
  output$comparing_immig_residuals_plot <- plotly::renderPlotly({
    ...
    p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = .data$age, y = .data[[y_val]])) +
      ...
      ggplot2::geom_point(size = 2) +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::labs(title = paste("Immigration Comparison Residuals", input$residual_type), y = y_lab) +
      ggplot2::theme_minimal()
    plotly::ggplotly(p)
  })
  ...
  output$comparing_emig_residuals_plot <- plotly::renderPlotly({
    ...
    p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = .data$age, y = .data[[y_val]])) +
      ...
      ggplot2::geom_point(size = 2) +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::labs(title = paste("Emigration Comparison Residuals", input$residual_type), y = y_lab) +
      ggplot2::theme_minimal()
    plotly::ggplotly(p)
  })
  ...
  output$comparing_agg_pop_plot <- plotly::renderPlotly({
    ...
    p <- ggplot2::ggplot(pop_aggs, ggplot2::aes(x = .data$time, y = .data$agg_pop, color = .data$setup)) +
      ...
      ggplot2::geom_point(size = 2) +
      ggplot2::facet_grid(cols = ggplot2::vars(.data$sex)) +
      ggplot2::labs(title = "Aggregate Population Comparison", y = "Total Population") +
      ggplot2::theme_minimal()
    plotly::ggplotly(p)
  })
  ...
  output$comparing_agg_immig_diffs_plot <- plotly::renderPlotly({
    ...
    p1_data <- mig_aggs %>%
      dplyr::select(time, sex, value = abs_diff) %>%
      dplyr::mutate(type = "Absolute")
    p2_data <- mig_aggs %>%
      dplyr::select(time, sex, value = perc_diff) %>%
      dplyr::mutate(type = "Percent")
  ...
  # TODO: Replicate similar logic for Emigration and Net Migration aggregate differences.
  ##############################################################################
  ...
  observe({ # Update data model sensitivity options based on checkbox group
    req(input$sa_datamod_name_select, input$sa_datamod_name_select != "None")
    if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_exact") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
    ...
    } else if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_norm") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
          "Count Scaler" = "count_scaler",
          ...
          "Constant SD" = "sd_overide"
        ),
        selected = "None"
      )
    } else if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_t") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
          "Count Scaler" = "count_scaler",
    ...
    } else if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_nbinom") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
          "Count Scaler" = "count_scaler",
    ...
    } else if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_poisson") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
          "Count Scaler" = "count_scaler",
    ...
    } else if (class(datamod_list()[[input$sa_datamod_name_select]])[1] == "accountTMB_datamod_lognorm") {
      updateSelectInput(session, "sa_datamod_param_select",
        choices = c(
          "None",
          "Count Scaler" = "count_scaler",
  ...
  output$sa_datamod_param_range_ui <- renderUI({
    ...
    if (input$sa_datamod_param_select == "scale_ratio") {
      ...
      default_steps <- 3
      step_val <- 1
    } else if (input$sa_datamod_param_select == "ratio") {
      default_min <- 0.9
      default_max <- 1.1
      default_steps <- 3
      step_val <- 0.1
    } else if (input$sa_datamod_param_select == "sd_overide") {
      default_min <- 1
      default_max <- 3
      default_steps <- 3
      step_val <- 1
    } else if (input$sa_datamod_param_select == "disp") {
      default_min <- 0.1
      default_max <- 0.5
      default_steps <- 5
      step_val <- 0.1
  ...
  observeEvent(input$run_sensitivity_analysis, {
    ...
    if (input$sa_sysmod_param_select == "None" && input$sa_datamod_param_select == "None") {
      shinyWidgets::sendSweetAlert(session, title = "No Parameter Selected", text = "Please select a system or data model parameter to vary.", type = "warning")
      return()
    }
    if (input$sa_datamod_param_select != "None" && input$sa_datamod_name_select == "None") {
      shinyWidgets::sendSweetAlert(session, title = "No Data Model Target", text = "Please select a specific data model to apply the parameter change to.", type = "warning")
      return()
    }
    ...
    param_values <- NULL
    varied_param_type <- NULL
    varied_param_name_full_id <- NULL
    sys_model_component_to_vary <- NULL
    data_model_to_vary <- NULL
    varied_param_name_short_id <- NULL
    ...
    if (input$sa_sysmod_param_select != "None") {
      ...
      if (input$sa_sys_param_min >= input$sa_sys_param_max || input$sa_sys_param_steps < 2) {
        shinyWidgets::sendSweetAlert(session, title = "Invalid Range", text = "System model parameter range or steps are invalid.", type = "error")
        return()
      }
      ...
      for (i in seq_along(param_values)) {
        ...
        if (varied_param_type == "sys") {
          ...
          if (is.null(target_sm_component_obj)) {
            shinyWidgets::sendSweetAlert(session, title = "Error", text = paste("Sys comp", sys_model_component_to_vary, "not found."), type = "error")
            return()
          }
          ...
          if (varied_param_name_short_id == "lower_rate_limit") {
            if ("mean" %in% names(target_sm_component_obj) && ("Counts" %in% class(target_sm_component_obj$mean) || is.data.frame(target_sm_component_obj$mean))) {
              ...
              if (is.null(rate_col_name)) {
                warning("Rate column not found in system model mean component.")
                next
              }
              ...
              warning(paste("Cannot apply lower_rate_limit to", sys_model_component_to_vary))
              next
            }
            ...
            if ("mean" %in% names(target_sm_component_obj) && ("Counts" %in% class(target_sm_component_obj$mean) || is.data.frame(target_sm_component_obj$mean))) {
              ...
              rate_col_name <- if ("rate" %in% names(rates_data)) "rate" else if ("mean" %in% names(rates_data)) "mean" else NULL
              if (is.null(rate_col_name)) {
                warning("Rate column not found for rate_scale.")
                next
              }
              ...
              warning(paste("Cannot apply rate_scale to", sys_model_component_to_vary))
              next
            }
            ...
            if ("mean" %in% names(target_sm_component_obj) && ("Counts" %in% class(target_sm_component_obj$mean) || is.data.frame(target_sm_component_obj$mean))) {
              ...
              rate_col_name <- if ("rate" %in% names(rates_data)) "rate" else if ("mean" %in% names(rates_data)) "mean" else NULL
              if (is.null(rate_col_name)) {
                warning("Rate column not found for rate_noise")
                next
              }

              rates_data[[rate_col_name]] <- rates_data[[rate_col_name]] + (stats::rnorm(length(rates_data[[rate_col_name]]), mean = 0, sd = current_param_val) * rates_data[[rate_col_name]])
              if ("Counts" %in% class(target_sm_component_obj$mean)) {
                target_sm_component_obj$mean <- Demographics::Counts(array(rates_data[[rate_col_name]], dim = dim(target_sm_component_obj$mean), dimnames = dimnames(target_sm_component_obj$mean)))
              } else {
                target_sm_component_obj$mean <- rates_data
              ...
              warning(paste("Cannot apply rate_noise to", sys_model_component_to_vary))
              next
            }
          } else if (varied_param_name_short_id == "dispersion") {
            target_sm_component_obj$disp <- current_param_val
        ...
        if (varied_param_type == "data") {
          target_dm_obj <- current_data_models[[data_model_to_vary]]
          if (is.null(target_dm_obj)) {
            shinyWidgets::sendSweetAlert(session, title = "Error", text = paste("Data model", data_model_to_vary, "not found."), type = "error")
            return()
          }
          target_dm_obj_modified <- target_dm_obj
          if (varied_param_name_short_id == "scale_ratio") {
            target_dm_obj_modified$scale_ratio <- current_param_val
          } else if (varied_param_name_short_id == "min_sd") {
            target_dm_obj_modified$sd <- target_dm_obj_modified$sd %>% dplyr::mutate(sd = ifelse(sd < current_param_val, current_param_val, sd))
          } else if (varied_param_name_short_id == "sd_scaler") {
            target_dm_obj_modified$sd <- target_dm_obj_modified$sd %>% dplyr::mutate(sd = sd * current_param_val)
          } else if (varied_param_name_short_id == "sd_overide") {
            target_dm_obj_modified$sd <- target_dm_obj_modified$sd %>% dplyr::mutate(sd = current_param_val)
          } else if (varied_param_name_short_id == "count_scaler") {
            target_dm_obj_modified$data <- target_dm_obj_modified$data %>% dplyr::mutate(count = count * current_param_val)
          } else if (varied_param_name_short_id == "ratio") {
            if (is.data.frame(target_dm_obj_modified$ratio)) {
              target_dm_obj_modified$ratio <- target_dm_obj_modified$ratio %>% dplyr::mutate(ratio = current_param_val)
            } else if (is.integer(target_dm_obj_modified$ratio)) {
              target_dm_obj_modified$ratio <- current_param_val
            }
          } else if (varied_param_name_short_id == "disp") {
            target_dm_obj_modified$disp <- current_param_val
          } else {
            warning(paste("Param", varied_param_name_short_id, "not in", data_model_to_vary))
            next
          }
          current_data_models[[data_model_to_vary]] <- target_dm_obj_modified
        }
        ...
        tryCatch(
          ...
          error = function(e) {
            shiny::showNotification(paste("Est. Error iter", i, ":", e$message), type = "error", duration = 10)
            run_result_obj <<- NULL
          }
        )
        ...
        if (!is.null(run_result_obj)) {
          ...
          if (!is.null(pop_res_df)) {
            pop_res_df[[param_info_current$name]] <- current_param_val
            pop_res_df$run_id <- i
            results_collector[[paste0("pop_run_", i)]] <- pop_res_df
          }
          if (!is.null(mig_res_df)) {
            mig_res_df[[param_info_current$name]] <- current_param_val
            mig_res_df$run_id <- i
            results_collector[[paste0("mig_run_", i)]] <- mig_res_df
          }
        }
      }
    })
    ...
    output$sensitivity_run_status <- renderText(paste("Sensitivity analysis complete. Successfully processed ", length(results_collector) / 2, " of ", length(param_values), " iterations."))
    ...
    if (!is.null(param_info_final)) {
      ...
      first_pop_res <- NULL
      iter_names <- names(results_collector)
      for (res_iter_name in iter_names) {
        if (startsWith(res_iter_name, "pop_run_")) {
          first_pop_res <- results_collector[[res_iter_name]]
          break
        }
      }
      if (!is.null(first_pop_res) && "time" %in% colnames(first_pop_res)) {
        unique_times_sa_pop <- sort(unique(first_pop_res$time))
        unique_times_sa_mig <- unique_times_sa_pop[c(-1)]
        updateSelectInput(session, "sa_pop_time_select", choices = unique_times_sa_pop, selected = unique_times_sa_pop[1])
        updateSelectInput(session, "sa_mig_time_select", choices = unique_times_sa_mig, selected = unique_times_sa_mig[1])
      } else {
  ...
  sa_combined_pop_results <- reactive({
    ...
    if (length(pop_dfs_list) == 0) {
      return(NULL)
    }
    dplyr::bind_rows(pop_dfs_list)
  })
  ...
  sa_combined_mig_results <- reactive({
    ...
    if (length(mig_dfs_list) == 0) {
      return(NULL)
    }
    dplyr::bind_rows(mig_dfs_list)
  })

  # Plot population estimates sensitivity analysis results
  output$sa_population_plot_output <- renderPlotly({
    ...
    if (!varied_param_col %in% colnames(plot_data_val)) {
      return(NULL)
    }
    ...
    p <- ggplot2::ggplot(plot_data_val, ggplot2::aes(x = .data$age, y = .data$population.fitted, color = .data[[varied_param_col]], group = .data[[varied_param_col]])) +
      ...
      ggplot2::theme_minimal() +
      ggplot2::guides(fill = "none") # Hide ribbon legend if color is same
    plotly::ggplotly(p)
  })

  # Plot immigration estimates sensitivity analysis results
  ...
  # Plot emigration estimates sensitivity analysis results
